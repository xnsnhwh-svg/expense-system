      ...
      <div className="glass rounded-2xl shadow-glass overflow-hidden">
        <div className="px-6 py-4 border-b border-warm-200/50 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-warm-400 mt-1">回收站中的记录不会出现在主列表中，可恢复或彻底删除</p>
          </div>
          {items.length > 0 && (
            <button
              onClick={handleDeleteAll}
              className="flex items-center gap-1.5 text-xs text-red-500 hover:text-red-700 hover:bg-red-50 font-medium px-3 py-2 rounded-xl transition-colors"
            >
        ...
        {loading ? (
          <div className="p-12 space-y-4">
            {[...Array(3)].map((_, i) => (<div key={i} className="h-12 animate-shimmer rounded-xl" />))}
          </div>
        ) : items.length === 0 ? (
          <div className="py-20 text-center">
            <ArchiveRestore className="w-12 h-12 text-warm-300 mx-auto mb-3" />
            <p className="text-warm-400 text-sm">回收站为空</p>
            <button
              onClick={() => navigate('/employee')}
            ...
            <table className="w-full">
              ...
              <tbody>
                {items.map((record, i) => (
                  ...
                  >
                    <td className="px-6 py-3.5 text-sm text-warm-600 text-warm-500 font-mono tracking-tight">{record.expense_no}</td>
                    <td className="px-6 py-3.5 text-sm font-semibold text-warm-800">¥{record.amount?.toFixed(2)}</td>
                    <td className="px-6 py-3.5 text-sm text-warm-600">{record.category}</td>
                    <td className="px-6 py-3.5"><StatusBadge status={record.status} /></td>
                    <td className="px-6 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => handleRestore(record.id)}
                          className="text-xs text-brand-500 hover:text-brand-700 hover:bg-brand-50 font-medium px-2.5 py-1.5 rounded-xl transition-colors flex items-center gap-1"
                        >
                          <Undo2 className="w-3 h-3" /> 恢复
                        </button>
                        <button
                          onClick={() => handlePermanentDelete(record.id)}
                          className="text-xs text-red-500 hover:text-red-700 hover:bg-red-50 font-medium px-2 py-1.5 rounded-xl transition-colors"
                        >
